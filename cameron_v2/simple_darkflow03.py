...
class SimpleDarkflow(object):
    ...
    def return_predict(self, meta, image):
        testimgfolder = "/home/salmis10/ProjectArchives/Archives/MachineLearning/YOLO/YOLO_from_scratch/Exp2018_1124_01/Project19/src/example01/cameron_v2/"
        image = cv2.imread(os.path.join(testimgfolder, 'grace_hopper.jpg'))
        if self.prepare:
            self._prepare_predict(meta)
        ho, wo, co = image.shape
        reshaped = reshape_image(image)
        print("first element of the input: ", reshaped[0,0,:])
        ...
        print(out[0].shape)
        print(type(out[0]))
        print("length of output: ", out[0].flatten().shape[0])
        print("first element: ", out[0].flatten()[0])
        print("last element: ", out[0].flatten()[13*13*36-1])
        print("output data type: ", out[0].flatten().dtype)
        ...
        if cimg.shape[0] != 0 and cimg.shape[1] != 0:
            ...
            cimg = puc.stretch_contrast(cimg)
    ...
    def _prepare_predict(self, meta):
        ...
        with self.graph.as_default():
            ...
            self.sess.run(tf.global_variables_initializer())
            # the argument is variables you want to save.
            self.saver = tf.train.Saver(tf.global_variables())
            ...
            # writer = tf.summary.FileWriter('../tensorboard/tb', tf.get_default_graph())


            # all_nodes = [n.name for n in tf.get_default_graph().as_graph_def().node]
            output_nodes = ['network_out']
            # print(all_nodes)
            output_graph_def = tf.graph_util.convert_variables_to_constants(
                                                self.sess,
                                                tf.get_default_graph().as_graph_def(),
                                                output_nodes)
            with tf.gfile.GFile("./frozen/frozen_model.pb", "wb") as f:
                f.write(output_graph_def.SerializeToString())
                print("protobuf file was successfully written")

            # Here, we save all varialbes and the default graph into some
            # files. The variables are saved into .data file, and the graph
            # is saved into .meta file.
            self.saver.save(self.sess, './frozen/data_all')
